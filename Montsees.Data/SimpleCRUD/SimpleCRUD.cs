...
namespace Dapper
{
	...
	public static class SimpleCRUD
	{
		...
		public static T Get<T>(this IDbConnection connection, object id)
		{
			...
			return connection.Query<T>(sb.ToString(), new IdContainer<Object>() { Id = id }).FirstOrDefault();
		}
		...
		public static int? Insert(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
		{
			...
			connection.Execute(sb.ToString(), entityToInsert, transaction, commandTimeout, CommandType.TableDirect);
			return (int?)connection.Query<IdContainer<int>>("select @@IDENTITY Id", null, transaction, true, commandTimeout, CommandType.Text).First().Id;
		...
		public static int Update(this IDbConnection connection, object entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null)
		{
			...
			return connection.Execute(sb.ToString(), entityToUpdate, transaction, commandTimeout,CommandType.Text);
		}
		...
		public static int Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null)
		{
			...
			return connection.Execute(sb.ToString(), entityToDelete, transaction, commandTimeout, CommandType.Text);
		}
		...
		public static int Delete<T>(this IDbConnection connection, object id, IDbTransaction transaction = null, int? commandTimeout = null)
		{
			...
			return connection.Execute(sb.ToString(), new IdContainer<object> { Id = id }, transaction, commandTimeout, CommandType.Text);
		}
		...
		private static bool IsEditable(PropertyInfo pi)
		{
			...
			if (attributes.Length > 0)
			{
				...
				EditableAttribute write = (EditableAttribute) attributes.FirstOrDefault(x => x.GetType().Name == "EditableAttribute");
				if (write != null)
				{
		...
		private static string GetTableName(Type type)
		{
			...
			TableAttribute tableattr = type.GetCustomAttributes(true).SingleOrDefault(attr => attr.GetType().Name == "TableAttribute") as TableAttribute;
			if (tableattr != null)
